(ns mieza.meanings.records.cluster-result-test
  ...
  (:require
   ...
   [mieza.meanings.records.clustering-state :refer [map->KMeansState]]
   [mieza.meanings.records.cluster-result :refer [load-model map->ClusterResult]]
   [mieza.meanings.kmeans]
...
(deftest test-save-and-load
  (let [test-centroid-dataset (test-dataset)
    ...
    (testing "That the ClusterResult record can be saved."
      (let [filename "test/mieza/meanings/records/cluster_result_test.edn"]
